...
use crate::model::{
    Fragment, FragmentKind, FragmentLocator, NarrationBlock, NarrationProfile, PronunciationRule,
};
...
pub fn plan_fragments(
    ...
    for block in blocks {
        ...
        {
            let kind = if block.kind == FragmentKind::Sentence && looks_like_dialogue(&sentence) {
                FragmentKind::Dialogue
            } else {
                block.kind.clone()
            };
            ...
            let mut fragment = Fragment {
                ...
                locator: FragmentLocator {
                    ...
                    css_selector: Some(format!("#ag-block-{}", &source_text_hash[..12])),
                    text_occurrence: block.occurrence,
                    source_text_hash,
                    cfi: None,
                },
                pause_after_ms,
...
pub fn split_sentences(text: &str) -> Vec<String> {
    ...
    while index < chars.len() {
        ...
        let boundary =
            matches!(character, '.' | '!' | '?') && !is_abbreviation_boundary(&chars, index);
        if boundary {
            let mut end = index + 1;
    ...
    if start < chars.len() {
        push_chunked(&mut output, chars[start..].iter().collect::<String>());
    }
    output
}
...
fn push_chunked(output: &mut Vec<String>, sentence: String) {
    ...
    if sentence.len() <= MAX_FRAGMENT_CHARS {
    ...
    while remaining.len() > MAX_FRAGMENT_CHARS {
        let safe = remaining
            .char_indices()
            .take_while(|(index, _)| *index <= MAX_FRAGMENT_CHARS)
...
fn is_abbreviation_boundary(chars: &[char], index: usize) -> bool {
    ...
    let before: String = chars[..index]
        ...
        .collect();
    let common = [
        "Mr", "Mrs", "Ms", "Dr", "Prof", "St", "No", "Jr", "Sr", "vs", "etc", "e", "i",
    ];
    if common
        .iter()
        .any(|value| before.eq_ignore_ascii_case(value))
    {
        return true;
    }
    let previous = index.checked_sub(1).and_then(|value| chars.get(value));
    let next = chars.get(index + 1);
    matches!((previous, next), (Some(a), Some(b)) if a.is_ascii_digit() && b.is_ascii_digit())
        || before.len() == 1
}
...
mod tests {
    ...
    fn keeps_abbreviations_together() {
        assert_eq!(
            split_sentences("Dr. Reed left. Then she returned."),
            vec!["Dr. Reed left.", "Then she returned."]
        );
    }
    #[test]
    fn separates_dialogue() {
        assert_eq!(
            split_sentences("\"Go.\" He stayed."),
            vec!["\"Go.\"", "He stayed."]
        );
    }
}
