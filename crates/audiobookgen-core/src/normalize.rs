...
static MONEY: LazyLock<Regex> = LazyLock::new(|| {
    Regex::new(r"\$([0-9][0-9,]*)(?:\.([0-9]{2}))?").expect("valid regex")
});
static PERCENT: LazyLock<Regex> = LazyLock::new(|| {
    Regex::new(r"\b([0-9]+(?:\.[0-9]+)?)%").expect("valid regex")
});
static TIME: LazyLock<Regex> =
    LazyLock::new(|| Regex::new(r"\b([0-9]{1,2}):([0-9]{2})\b").expect("valid regex"));
static ROMAN_CHAPTER: LazyLock<Regex> = LazyLock::new(|| {
    Regex::new(r"(?i)\b(chapter|book|volume)\s+([ivxlcdm]+)\b").expect("valid regex")
});
...
pub fn normalize_for_speech(input: &str, rules: &[PronunciationRule]) -> String {
    ...
    text = MONEY
        .replace_all(&text, |caps: &Captures<'_>| money(caps))
        .into_owned();
    ...
    for rule in rules {
        ...
        } else if let Ok(regex) = Regex::new(&format!("(?i){}", regex::escape(&rule.pattern))) {
            text = regex
...
fn expand_common_abbreviations(input: &str) -> String {
    ...
    replacements
        .into_iter()
        .fold(input.to_owned(), |text, (pattern, replacement)| {
            Regex::new(pattern)
...
fn money(caps: &Captures<'_>) -> String {
    ...
    match caps.get(2).map(|value| value.as_str()).filter(|value| value != &"00") {
        Some(cents) => format!(
            "{amount} {unit} and {} cents",
            cents.parse::<u32>().unwrap_or(0)
        ),
...
mod tests {
    ...
    #[test]
    fn expands_bookish_text() {
        let output = normalize_for_speech(
            "Dr. Reed paid $12.50 at 8:05 — a 25% rise.",
            &[],
        );
        assert_eq!(
            output,
            "Doctor Reed paid 12 dollars and 50 cents at 8 oh 5 — a 25 percent rise."
        );
    }

    #[test]
    fn expands_roman_chapters() {
        assert_eq!(normalize_for_speech("Chapter XIV", &[]), "Chapter 14");
    }
}
