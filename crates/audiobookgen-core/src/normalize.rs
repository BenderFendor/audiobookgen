...
static MONEY: LazyLock<Regex> =
    LazyLock::new(|| Regex::new(r"\$([0-9][0-9,]*)(?:\.([0-9]{2}))?").expect("valid regex"));
static PERCENT: LazyLock<Regex> =
    LazyLock::new(|| Regex::new(r"\b([0-9]+(?:\.[0-9]+)?)%").expect("valid regex"));
...
pub fn normalize_for_speech(input: &str, rules: &[PronunciationRule]) -> String {
    ...
    text = MONEY
        .replace_all(&text, |caps: &Captures| money(caps))
        .into_owned();
