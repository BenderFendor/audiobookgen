...
pub struct SleepGuard { child: Option<Child> }

impl SleepGuard {
    pub fn acquire() -> Self {
        let child = platform_command().and_then(|mut command| {
            command.stdin(Stdio::null()).stdout(Stdio::null()).stderr(Stdio::null()).spawn().ok()
        });
        Self { child }
    }
}
...
impl Drop for SleepGuard {
    fn drop(&mut self) {
        if let Some(child) = self.child.as_mut() { let _ = child.kill(); let _ = child.wait(); }
    }
}
...
fn platform_command() -> Option<Command> {
    ...
    command.args(["--what=sleep", "--why=AudiobookGen is generating narration", "--mode=block", "sleep", "infinity"]);
    Some(command)
}
...
fn platform_command() -> Option<Command> { None }
