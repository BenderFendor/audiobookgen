...
class TransformerBlock(nn.Module):
    ...
    def forward(self, x, freqs_cis=None, mask=None, cache=None, pos=0):
        h, new_cache = self.attention(
            self.attention_norm(x), freqs_cis, mask, cache, pos
        )
        x = x + h
...
class AcousticTransformer(nn.Module):
    def __init__(self, config: VoxtralConfig):
        ...
        self.input_projection = nn.Linear(
            config.n_acoustic_codebooks, config.dim, bias=False
        )
        ...
        self.semantic_codebook_output = nn.Linear(
            config.dim,
            config.semantic_codebook_size + config.special_count + 126,
            bias=True,
        )
        # Output is 8320 = 8192 + 2 + 126 padding
        self.acoustic_codebook_output = nn.Linear(
            config.dim, config.n_acoustic_codebooks, bias=False
        )
    ...
    def predict_velocity(self, x_t, llm_hidden, t: float):
        ...
        tok_time = self.time_projection(
            self.time_embedding(t, device).unsqueeze(0).expand(B, -1).to(dtype)
        )
    ...
    def decode_one_frame(self, llm_hidden: torch.Tensor):
        ...
        x_t = (
            torch.randn(B, config.n_acoustic_codebooks, device=device)
            * config.sigma_max
        )
...
class CodecQKNorm(nn.Module):
    ...
    def forward(self, q, k):
        ...
        q = (
            self.q_norm(q.transpose(1, 2).reshape(B, T, H * D))
            .reshape(B, T, H, D)
            .transpose(1, 2)
        )
        k = (
            self.k_norm(k.transpose(1, 2).reshape(B, T, H * D))
            .reshape(B, T, H, D)
            .transpose(1, 2)
        )
        return q, k
...
class CodecAttention(nn.Module):
    ...
    def __init__(self, dim, n_heads, head_dim, window_size, eps=1e-2):
        ...
        # ALiBi slopes
        slopes = torch.tensor(
            [2.0 ** (-8.0 / n_heads * (h + 1)) for h in range(n_heads)]
        )
...
class CodecDecoder(nn.Module):
    def __init__(self, config: VoxtralConfig):
        ...
        self.config = config
        code_dim = (
            config.semantic_embed_dim + config.n_acoustic_codebooks
        )  # 256 + 36 = 292
            ...
            stage = nn.ModuleList(
                [
                    CodecTransformerLayer(
                        config.codec_dim,
                        config.codec_n_heads,
                        config.head_dim,
                        config.codec_hidden_dim,
                        ws,
                        config.codec_norm_eps,
                    )
                    for _ in range(2)
                ]
            )
        ...
        # Output projection
        self.output_proj = WeightNormConv1d(
            config.codec_dim, config.patch_size, kernel_size=7
        )
    ...
    def embed_codes(self, codes):
        ...
        sem_embed = self.semantic_embedding_sum / self.semantic_cluster_usage.unsqueeze(
            1
        ).clamp(min=1e-8)
        ...
        acoustic_continuous = (
            acoustic_codes.float() * 2.0 / (self.config.fsq_levels - 1) - 1.0
        )
...
class VoxtralTTS(nn.Module):
    ...
    def embed_audio_codes(self, codes):
        ...
        embed_sum = torch.zeros(
            B, self.config.dim, device=codes.device, dtype=torch.bfloat16
        )
        ...
        for k in range(1, 37):
            offset = 8194 + (k - 1) * 23
            idx = (
                torch.nan_to_num(codes[:, k].float(), nan=0.0).long().clamp(0, 22)
                + offset
            )
