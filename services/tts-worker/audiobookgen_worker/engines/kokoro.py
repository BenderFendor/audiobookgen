...
class KokoroEngine:
    ...
    def ensure_model(
        ...
        download_with_progress(REPO_ID, model_dir, progress)
    ...
    def generate(
        ...
        if not voice_path.is_file():
            raise RuntimeError(
                f"Kokoro voice is missing from the installed model: {voice}"
            )
    ...
    def _get_pipeline(self, model_dir: Path, voice: str):
        ...
        if (
            ...
            if not config.is_file() or not weights.is_file():
                raise RuntimeError(
                    "Kokoro model files are incomplete; download the model again"
                )
            model = KModel(repo_id=REPO_ID, config=str(config), model=str(weights))
            self._pipeline = KPipeline(
                lang_code=lang_code, repo_id=REPO_ID, model=model
            )
